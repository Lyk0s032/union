...
export default function ResultBodegaProyecto({ item }){
    ...
    const filtro = item.cotizacion_compromisos.filter(c => c.cotizacionId == params.get('who'));

    const proyecto = filtro[0]
    const close = () => {
        setTransferir(false);
    }

    useEffect(() => {
        if(params.get('show') == 'Proyecto'){
            dispatch(actions.axiosToGetItemMateriaPrimaBodegaProyecto(true, params.get('item'), Number(params.get('who'))))
            console.log('consultando por proyecto')
        }
    }, [params.get('who'), params.get('show')])
    console.log('por proyecto', item)
            ...
            <div className="containerHere">
                <div className="hereTitle">
                    <div className="" onDoubleClick={() => {
                        ...
                        <h3>{proyecto?.cotizacion?.name} - #{proyecto?.cotizacion?.id}</h3>
                ...
                <div className="howMany">
                    <div className="howManyItem">
                        <h1>{Number(proyecto?.cantidadEntregada)}</h1>
                        <span>Cantidad</span>
                    </div>
                    <div className="howManyItem">
                        <h1>{Number(proyecto?.cantidadComprometida)}</h1>
