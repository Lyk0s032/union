...
export default function ProviderAnalisis(){
    ...
    const searchProviders = async () => {
        try{
            setLoading(true)
            let body = {
                productoId: params.get('MP'),
                proveedores: proveedoresArray 
            }

            const searchThat = await axios.post('/api/requisicion/post/searchProviders/analisis/productos', body)
            .then((res) => {
                setData(res.data);
            })
            .catch((err) => { 
    ...
    const updatePrice = async (proveedor) => {
        ...
        if(valor == proveedor?.productPrices[0].descuentos) return dispatch(actions.HandleAlerta('Debes ingresar un valor diferente'))
                    ...
                    <div> 
                        {
                            data?.map((pv, i) => {
                                ...
                                let lastUpdate = dayjs(proveedor?.productPrices[0].createdAt).format("dddd, D [de] MMMM YYYY, h:mm A");
                                                        ...
                                                        <div className="nameAndLetter">
                                                            ...
                                                            <div className="dataAndOther">
                                                                ...
                                                                <div className="price">
                                                                    ...
                                                                    <h1>$ {new Intl.NumberFormat('es-CO', {currency:'COP'}).format(Number(proveedor?.productPrices[0].valor).toFixed(0))}</h1>
