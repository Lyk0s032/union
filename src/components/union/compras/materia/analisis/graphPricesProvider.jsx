...
// ------------------------------------------------------------------
export default function GraphProviderPrices() {
  ...
  const materiaId = params.get('prima');
  ...
  const fetchPriceHistory = async (materiaId, proveedorId) => {
    ...
    try {
      ...
      const { data: historyData } = await axios.get(
        `/api/materia/get/prices/all/${materiaId}/${proveedorId}`
      );
      ...
      <div className="containerGraphPrices">
        <div className="headerModal">
          <div className="divideThat">
            <h3>{providerInfo.nombre}</h3>
            <span>NIT: {providerInfo.nit}</span>
          </div>
          <div className="close">
            <button onClick={() => {
              params.delete('graph');
